...
from basketball_reference_web_scraper.output import box_scores_to_csv
from basketball_reference_web_scraper.output import output
from basketball_reference_web_scraper.output import players_season_totals_to_csv
from basketball_reference_web_scraper.output import schedule_to_csv
from basketball_reference_web_scraper.output import team_box_scores_to_csv
...
def team_box_scores(day, month, year, adv=False, output_type=None, output_file_path=None, output_write_option=None, json_options=None):
    try:
        values = http_client.team_box_scores(day=day, month=month, year=year, adv=adv)
    except requests.exceptions.HTTPError as http_error:
        if http_error.response.status_code == requests.codes.not_found:
            raise InvalidDate(day=day, month=month, year=year)
        else:
