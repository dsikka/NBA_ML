...
from basketball_reference_web_scraper.parsers.box_scores.teams import parse_team_adv_totals
...
from basketball_reference_web_scraper.parsers.schedule import parse_schedule
from basketball_reference_web_scraper.parsers.schedule import parse_schedule_for_month_url_paths
...
def team_box_scores_adv(game_url_path):
    print('Here')
    url = "{BASE_URL}/{game_url_path}".format(BASE_URL=BASE_URL, game_url_path=game_url_path)
    ...
    return parse_team_adv_totals(response.content)


def team_box_scores(day, month, year, adv=False):
    ...
    if adv:
        ...
        for box_score in team_box_scores_adv(game_url_path=game_url_path)
    ]
    else:
        return [
            box_score
